...
The dashboard is a **separate process** — the owner's live window onto the traces. It does not share memory with the engine; it polls the `observations` (and `events`) table by rowid and streams new rows to the browser. It is the durable, always-available default view: a task timeline, the phase and sub-phase tree, the decisions with their alternatives and reasoning, the agent runs — each expanding into the agent's own conversation (its messages, thinking, tool calls and results), played live while the run is in flight and re-watchable once it is past — the verdicts, the errors — each expandable down to the underlying blob. Because it reads the system of record rather than a live feed, it shows history as readily as the present, and it survives restarts. Because it is a separate process, anything held only in a daemon component's memory is invisible to it: the periodic background sweeps, for instance, each publish a durable per-sweep completion event so the dashboard can surface recent cleanup, since their in-memory last-run summary is unreachable across the process boundary. The workspace reaper publishes `system.reap_completed` (the **Cleanup** card) and the data-lifecycle manager publishes `system.cleanup_completed` (the **Data Lifecycle** card); a sweep that pruned nothing still publishes its completion event, so the card proves the service is alive rather than going stale.
...
- **Edge-trigger transitions, never heartbeat them.** A state change emits once, on the transition — the crossing into a threshold, a window rollover, a plugin going unhealthy — and re-arms only when the state genuinely changes back. A naive per-event check that re-emits on every event while the condition still holds (every cost event once spend is past 80%, every tick a plugin is still down) buries the real transition under repeats. The cost tracker, for example, records the 80%-of-limit crossing and the daily/monthly window rollover exactly once per occurrence, latched until the window resets.
