...
function insertTask(db: Database.Database, id: string, state: string, overrides: TaskOverrides = {}): void {
  db.prepare(
    `INSERT INTO tasks (id, idempotency_key, state, sub_state, priority, title, created_at, last_transition_at,
       completed_at, not_before, consecutive_crash_count, consecutive_agent_unavailable_count, blocked)
...
describe("retryTask", () => {
  ...
  it("retries each retryable state and refuses every non-retryable state", () => {
    for (const state of [TaskStates.failed, TaskStates.blocked]) {
