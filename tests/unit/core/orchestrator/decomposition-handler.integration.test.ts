...
describe("Orchestrator — Decomposition", () => {
  ...
  it("creates child tasks when planning returns a valid decomposition plan", async () => {
    ...
    expect(h.sessionMemory.endSession).toHaveBeenCalledWith(expect.any(String), SessionEndReasons.decomposed);
  });
        ...
        child_summaries: [
          {
            ...
            key_outputs: [{ type: "file" as const, path: "src/schemas.ts", description: "New schemas" }],
            patterns_introduced: [],
            gotchas: [],
            decisions_made: [],
            pr_number: 5,
