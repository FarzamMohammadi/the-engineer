...
import type { SafetyQuery } from "../../../../src/core/interfaces/safety-layer.interface.js";
import {
  PolicyEngine,
  evaluateThreshold,
  matchesPathPattern,
  parseThreshold,
} from "../../../../src/core/safety-layer/policy-engine.js";
...
describe("PolicyEngine — evaluateAutonomy", () => {
  ...
  it("threshold within limit returns proceed", () => {
    ...
    const verdict = engine.evaluateAutonomy(makeQuery({ decision_category: "refactoring", details: { scope: 3 } }));
    expect(verdict.allowed).toBe(true);
  });
  ...
  it("repo override takes precedence over base config", () => {
    ...
    const baseVerdict = engine.evaluateAutonomy(makeQuery({ repo: "owner/other", decision_category: "refactoring" }));
...
describe("PolicyEngine — hot-reload", () => {
  it("new scope rules apply after updateConfig", () => {
    ...
    // Initially denied
    expect(engine.evaluateScope(ActionClasses.write, { repo: "owner/repo-b" })?.action).toBe("deny");
