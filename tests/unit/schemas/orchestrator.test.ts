...
describe("FileChangeSchema", () => {
  ...
  it("accepts all change types", () => {
    for (const ct of ["create", "modify", "delete"]) {
      expect(FileChangeSchema.parse({ file: "x", change_type: ct, description: "y" }).change_type).toBe(ct);
    }
  });
});
...
describe("RiskSchema", () => {
  it("parses valid data", () => {
    expect(RiskSchema.parse({ risk: "API breaking change", mitigation: "Version the endpoint" })).toBeDefined();
  });
});
...
describe("SelfReviewFindingSchema", () => {
  ...
  it("accepts all finding types", () => {
    for (const type of ["style", "logic", "performance", "security", "maintainability"]) {
      expect(SelfReviewFindingSchema.parse({ type, file: "x", description: "y", fixed: false }).type).toBe(type);
    }
  });
});
