...
## External Trace Export — Auto-Fetch & Supervise the Backend

**Built:** The export itself now ships. Every dispatch is projected, opt-in, into any OTLP/HTTP backend (Jaeger v2, the OTel Collector, Tempo, Honeycomb) as a poll-based, best-effort reader of the SQLite observation tree — one swappable endpoint, sanitized at the export boundary, with a dashboard deep-link and a doctor check. See the live docs: [the observability how-to's External Trace Export section](contribution-docs/how-tos/observability.md#external-trace-export-otlp) and [the telemetry configuration](configuration/daemon.md#telemetry). What follows is the one piece that was deliberately *deferred*.

**Current state:** The owner brings their own backend. The Engineer points at an OTLP endpoint and prints an OS-aware install pointer when nothing answers, but it does **not** download, install, run, or supervise the backend binary. `brew install jaeger && jaeger`, the official download, or `docker run` is a manual, one-time step.

**Why deferred:** The projection is the full capability and works against any user-run backend, so fetching the binary is an off-by-default convenience — and the riskiest code to hand-roll for one. The supply-chain surface (download, checksum-verify, extract, cache, cross-platform) is real, version-pinning and soak machinery would have to come with it, and unlike the in-process dashboard there is no precedent in the system for supervising a fetched, transient daemon. The panel was unanimous: not worth it in the export slice.

**When it becomes relevant:** When standing up the backend is the recurring friction that stops owners from using export at all — or, more naturally, when npm distribution solves binary-fetch once for the whole project and this can ride along on that solved mechanism rather than hand-rolling its own.
