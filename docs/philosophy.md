...
1. **[Real Engineer Behavior](#real-engineer-behavior) applied.**
   - Did you understand the problem before acting, or did you jump to implementation?
   - Did you research the codebase for existing mechanisms, patterns, and utilities that could be reused or extended — before writing anything new?
   ...
   - Where confidence is partial, is uncertainty surfaced explicitly — not hidden?

2. **[Post-Completion Rigor](#post-completion-rigor) applied.**
...
4. **Type checks clean.** `tsc --noEmit` returns zero.
...
7. **Docs updated.** Every changed contract, behavior, or flow has its corresponding doc update in the same unit of work — not "later," not "in a follow-up." A code change without its doc update is unfinished work, the same as a function without tests. This includes: adapter contract docs when methods change, plugin docs when implementations change, user flow docs when behavior changes, and configuration docs when options change. See [Docs as System Blueprint](#docs-as-system-blueprint) for what the docs must cover.
8. **Observability verified.** The three tests from [Radical Observability](#radical-observability--the-owner-is-never-in-the-dark) pass: debuggability, owner sync, external reach.
...
- **Always in sync.** Code changes and doc changes are the same unit of work. Every change to user-facing behavior updates docs in the same step. This is enforced through the [Definition of Done](#definition-of-done) — item 7 is not optional.
...
Two build-time guards enforce this so a regression cannot merge: [`tests/boundary/tier-import-rules.test.ts`](https://github.com/FarzamMohammadi/the-engineer/blob/main/tests/boundary/tier-import-rules.test.ts) catches a *structural* leak (Core importing a plugin), and [`tests/boundary/plugin-opacity.test.ts`](https://github.com/FarzamMohammadi/the-engineer/blob/main/tests/boundary/plugin-opacity.test.ts) catches a *semantic* one (a platform name as a string literal anywhere in `src/core/`). If you are typing a platform's name into Core, the second guard is about to fail — derive the value instead.
