...
describe("Observer", () => {
  ...
  describe("error secret sanitization", () => {
    it("recordError sanitizes token-bearing URLs in error_message field", () => {
      handle.observer.recordError(new Error("request to https://ghp_abc123xyz@api.github.com/repos failed"), {
    ...
    it("recordError sanitizes token-bearing URLs in input.error_message", () => {
      handle.observer.recordError(new Error("push to https://git:secret_token@github.com/org/repo.git failed"), {
        operation: "git_push",
  ...
  describe("storeBlob() / readBlob()", () => {
    ...
    it("works without blobStore (graceful degradation)", async () => {
      const { createObservationStore } = await import("../../../../src/core/observer/index.js");
