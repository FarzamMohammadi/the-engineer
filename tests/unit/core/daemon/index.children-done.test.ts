...
describe("Daemon — Decomposition", () => {
  ...
  describe("checkAndEmitChildrenAllDone", () => {
    it("emits children_all_done when last child completes", async () => {
      ...
      const payload = (childrenDoneEvent?.[0] as Record<string, unknown>)["payload"] as Record<string, unknown>;
  ...
  describe("slot overrun", () => {
    it("re-queues parent when all slots are full after children complete", async () => {
      ...
      const requeueCall = handle.taskEngine.requestTransition.mock.calls.find(
        (call: unknown[]) =>
          call[0] === "parent-1" && call[1] === TaskStates.queued && (call[3] as string).includes("slot_unavailable"),
      );
      expect(requeueCall).toBeDefined();

      // child_summaries should still be populated
      expect(handle.taskEngine.updateTaskField).toHaveBeenCalledWith("parent-1", "child_summaries", expect.any(Array));

      // Should NOT have transitioned to integrating
      const integratingCall = handle.taskEngine.requestTransition.mock.calls.find(
        (call: unknown[]) =>
          call[0] === "parent-1" && call[1] === TaskStates.active && call[2] === SubStates.integrating,
      );
