...
- Cost is not attributed for a run that ends in error: the spend it incurred before dying is not carried on the error, so it never reaches the cost ledger the budget guardrails read. This includes a run that errored *after* writing a valid `session-result.json` -- Core still prefers that work and the sub-phase completes normally, but the attempt books $0. Carrying spend on the adapter error would fix it and is a contract change, deliberately out of scope here.
