...
function buildRequirementsInstructions(ctx: RequirementsGatheringPromptContext): string {
  return section(
    ...
    [
      ...
      "",
      "6. **Deciding when to block vs. proceed:**",
      "   - **Block** (signal need_more_info) when: scope is ambiguous with multiple valid interpretations, security implications are unclear, breaking change potential exists, or multiple people are affected and need coordination.",
      "   - **Proceed** (signal ready) when: direction is clear even if implementation details are uncertain, you can make a reasonable choice and document it, or the task is well-scoped and self-contained.",
      "",
      "   When writing outreach files, **number your questions** so the recipient can answer each one clearly.",
      "",
      "   Signal ready only when you genuinely know what to build with zero assumptions. Otherwise signal need_more_info — The Engineer will read your outreach files and deliver them to each person via their preferred channel (Telegram, GitHub, etc.).",
      "",
