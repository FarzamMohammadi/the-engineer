...
describe("isValidTransition", () => {
  ...
  it("accepts requirements_gathering to failed", () => {
    expect(
      isValidTransition(TaskStates.requirements_gathering, null, TaskStates.failed, null),
    ).toBe(true);
  });
  ...
  it("rejects when to_sub is wrong", () => {
    ...
    expect(
      isValidTransition(TaskStates.queued, null, TaskStates.active, SubStates.supervising),
    ).toBe(false);
  });

  it("rejects completed to anything", () => {
    expect(
      isValidTransition(TaskStates.completed, null, TaskStates.active, SubStates.working),
    ).toBe(false);
