...
describe("ReviewHandler", () => {
  ...
  describe("checkFeedback", () => {
    ...
    it("emits new event when review state changes", async () => {
      ...
      expect((feedbackCalls[0]![0] as { payload: TaskFeedbackReceivedPayload }).payload.feedback_type).toBe(
        "changes_requested",
      );
      expect((feedbackCalls[1]![0] as { payload: TaskFeedbackReceivedPayload }).payload.feedback_type).toBe("approved");
    });
  ...
  describe("handleFeedbackEvent", () => {
    ...
    it("post-approval fix resets accommodated_review_state and accommodated_comment_ids", async () => {
      ...
      expect((reviewUpdate![2] as { accommodated_comment_ids: unknown[] }).accommodated_comment_ids).toEqual([]);
    });
    ...
    it("post-approval fix retry limit: completes with manual merge message", async () => {
      ...
      (ctx.taskEngine as unknown as { getStateHistory: ReturnType<typeof vi.fn> }).getStateHistory.mockReturnValue([
    ...
    it("stores feedback round on the task", () => {
      ...
      expect(reviewArg.feedback_rounds[0]!.comments).toEqual(["Line 10 needs a fix", "Line 20 also"]);
  ...
  describe("post-approval merge conflict detection", () => {
    ...
    it("handleCodeApproval: CI failing + mergeable false → grouped feedback with BOTH issues", async () => {
      ...
      const queuedCalls = te.requestTransition.mock.calls.filter((c: unknown[]) => c[1] === TaskStates.queued);
      expect(queuedCalls).toHaveLength(1);
      expect(queuedCalls[0]).toEqual(["task-1", TaskStates.queued, null, "post_approval_fix", "daemon"]);
  ...
  describe("feedback accommodation tracking", () => {
    ...
    it("updates accommodated state when new feedback is detected", async () => {
      ...
      const reviewUpdates = te.updateTaskField.mock.calls.filter((c: unknown[]) => c[1] === "review");
