...
> **Note:** `subStateMatches()` is currently a **private** (non-exported) helper function. If extracting to `state-machine.ts`, export it explicitly — it's `function subStateMatches()` in the current code, not `export function`.
