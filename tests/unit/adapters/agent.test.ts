...
import { AdapterErrorSeverities } from "../../../src/schemas/adapters.js";

class TestAgentAdapter extends AgentAdapter {
  inferResult: AgentRunResult = {
  ...
  protected doRun(_request: AgentRunRequest): Promise<AgentRunResult> {
    if (this.inferError) {
...
const testRequest: AgentRunRequest = {
  prompt: "Hello, world",
  system_prompt: null,
  cwd: null,
...
describe("AgentAdapter", () => {
  it("extends BaseAdapter", () => {
    const adapter = new TestAgentAdapter();
    expect(adapter).toBeInstanceOf(BaseAdapter);
    expect(adapter).toBeInstanceOf(AgentAdapter);
  });

  describe("infer (template-wrapped)", () => {
    it("returns AgentRunResult from doRun on success", async () => {
      const adapter = new TestAgentAdapter();
      adapter.manifest = createManifest();
      const result = await adapter.run(testRequest);
    ...
    it("rethrows AdapterMethodError as-is", async () => {
      const adapter = new TestAgentAdapter();
      adapter.manifest = createManifest();
      adapter.inferError = new AdapterMethodError(createAdapterError("context_exceeded", "Prompt too long"));

      try {
        await adapter.run(testRequest);
