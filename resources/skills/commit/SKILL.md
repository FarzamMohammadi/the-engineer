...
- `Fix race condition in concurrent task dispatch`
...
- **Never add Co-Authored-By** or other attribution lines — only the repository owner is credited
- **Never use `--no-verify`** — hooks exist to catch errors before CI does
- **Never ask questions** — analyze and commit
- **Match project style** — check recent commits for conventions before writing
