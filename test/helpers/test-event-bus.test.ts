...
describe("createTestEventBus", () => {
  ...
  it("assertEventEmitted throws when event not found", () => {
    ...
    expect(() => handle.assertEventEmitted("task.created")).toThrow(
      'Expected event "task.created" to be emitted',
    );
  });
  ...
  it("assertEventEmitted with payloadMatcher works", () => {
    ...
    expect(() =>
      handle.assertEventEmitted("task.created", (p) => p["title"] === "specific"),
    ).not.toThrow();

    expect(() =>
      handle.assertEventEmitted("task.created", (p) => p["title"] === "nonexistent"),
    ).toThrow("none matched the payload predicate");
  });
});
