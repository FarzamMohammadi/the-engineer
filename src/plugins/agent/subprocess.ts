/**
 * Shared subprocess discipline for agent plugins.
 *
 * Every agent plugin spawns a CLI child process. These helpers enforce the two
 ...
 *   adapters (`GITHUB_TOKEN`, `TELEGRAM_BOT_TOKEN`, ...) never reach the agent.
...
/** Environment variable allowlist for agent CLI child processes. */
...
const AGENT_ENV_PREFIX_ALLOWLIST = ["LC_"];

/**
 * Build a sanitized environment for agent child processes.
 * Only forwards allowlisted vars — no secrets leak to the subprocess.
 */
export function buildAgentEnv(env: NodeJS.ProcessEnv): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(env)) {
    ...
    if (allowed.has(key) || AGENT_ENV_PREFIX_ALLOWLIST.some((p) => key.startsWith(p))) {
      result[key] = value;
    }
  }
  return result;
