...
function buildDemoPrepInstructions(): string {
  return section(
    ...
    [
      ...
      "   - Frontend: describe UI changes, reference before/after state in code",
      "   - Backend: API examples, test output demonstrating behavior",
      "   - Infrastructure: config diffs, dry-run output",
      "   - Use run_command ONLY for quick commands (tests, builds, git diff)",
      "   - NEVER start dev servers, watch processes, or any long-running commands (npm run dev, npm start, etc.)",
      "",
