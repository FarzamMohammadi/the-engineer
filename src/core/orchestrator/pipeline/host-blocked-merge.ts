...
//
// One condition — the host reports `merge_state === "blocked"`: the PR is mergeable in
// *shape*, but branch protection will not let the Engineer complete the merge — is reached
// by two independent paths:
//
//   1. the PR-event poller, when an authorized `/approve` comment lands on a blocked PR
//      (a comment cannot satisfy a required *formal* review), and
//   2. delivery's auto-merge, in readiness (before any merge call) and in its merge-failure
//      backstop (the rare detect→merge race).
//
// Both resolve to the ONE contract defined here: the same block category, and the same
// honest message. This module exists so they cannot drift apart — that drift is exactly
// what this bug was (issue #47): the two paths resolved the same condition to opposite task
// lifecycle states and made opposite promises to the owner.
//
...
//     outcome the message promises (approve on the host, retry, and the merge finishes) becomes
//     *impossible*.
//   - `awaiting_human` collapses to the `need_more_info` block reason, which takes the task OFF
//     the `pr_review_pending` poll set — so the promote → doomed-merge → rework loop stays
//     structurally impossible, exactly as `done` did — and puts it ON the health monitor's
//     blocked-escalation ladder: the owner is nudged, a self-unblock is attempted once, and a
//     task that never delivers ends `failed` rather than falsely `completed`.
//
// Do not "simplify" this back into a completion.

/**
 * The single lifecycle target for a host-blocked merge, whichever path detects it. `awaiting_human`
 * ⇒ `need_more_info` ⇒ off the PR-review poll set (the structural loop bound) and onto the
 * blocked-escalation ladder (the honest terminal). The task stays resumable: once the owner approves
 * on the host, `engineer retry` re-checks and merges.
 */
export const HOST_BLOCKED_MERGE_CATEGORY = BlockCategories.awaiting_human;

/**
 * The one owner-facing message for a host-blocked merge — used verbatim as the block's `needed` by
 * both paths, so the owner reads the same thing however the block was detected.
 *
