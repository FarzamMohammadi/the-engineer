...
The Engineer picks up queued tickets in priority order. To nudge an urgent ticket ahead of the rest, put `@priority: <number>` anywhere in the ticket body:

> The login page 500s on every submit — production is down.
>
> @priority: 90

Higher numbers run first. The valid range is **1–100**, and the default is **50** when you leave it out — so use a number above 50 to jump the queue and below 50 to let other work go first. Anything outside 1–100 is ignored, and the ticket falls back to the default. One `@priority` per ticket is all you need.
