...
Skills in `resources/skills/` are the source-of-truth, synced to `{workspace_root}/skills/` at startup by `SkillsManager.sync()`. Phase prompts reference skills by absolute path to that runtime location. The CLI reads skill files on demand using its Read tool — no content is inlined into prompts.
...
Skills are mapped to specific pipeline phases:
- **execution** — `commit`
- **review** — `commit`, `expert-panel-review`
- Other phases (requirements, research, planning, delivery) receive no skills
