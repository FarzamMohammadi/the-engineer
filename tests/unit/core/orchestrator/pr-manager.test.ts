...
} from "../../../../src/core/orchestrator/pr-manager.js";
...
describe("createPullRequest", () => {
  ...
  it("returns rework_pushed and marks feedback as applied on rework path", async () => {
    ...
    const result = await pm.createPullRequest("session-001", "task-001", makeDemoPrepOutput(), dispatch);
  ...
  it("returns no_hosting_plugin when git hosting adapter is absent", async () => {
    ...
    const result = await pm.createPullRequest("session-001", "task-001", makeDemoPrepOutput(), createDispatch());
  ...
  it("creates PR and returns created with pr_number and url", async () => {
    ...
    (ctx.registry.getPrimaryPlugin as ReturnType<typeof vi.fn>).mockImplementation((type: string) =>
      type === "git_hosting" ? fakeGitHosting : null,
    );
    const pm = createPrManager(ctx, createMockNotifier());
  ...
  it("sanitizes PR title before sending to API", async () => {
    ...
    await pm.createPullRequest("session-001", "task-001", makeDemoPrepOutput({ pr_description: "Clean" }), dispatch);
  ...
  it("reads PR description from deliverable file when not in PhaseOutput.data", async () => {
    ...
    vi.mocked(existsSync).mockImplementation((p: unknown) => String(p).endsWith("pr-description.md"));
  ...
  it("includes trigger reference when external_ref has URL", async () => {
    ...
    const prArgs = fakeGitHosting.createPR.mock.calls[0]![0];
    expect(prArgs.body).toContain("> Triggered by [owner/repo#42](https://github.com/owner/repo/issues/42)");
  ...
  it("applies title_suffix from pr_decorations", async () => {
    ...
    expect(fakeGitHosting.createPR).toHaveBeenCalledWith(expect.objectContaining({ title: "Fix bug [urgent]" }));
  });
  ...
  it("applies both title_prefix and title_suffix", async () => {
    ...
    expect(fakeGitHosting.createPR).toHaveBeenCalledWith(expect.objectContaining({ title: "#42: Fix bug [urgent]" }));
  });
  ...
  it("does not decorate PR title when pr_decorations is absent", async () => {
    ...
    expect(fakeGitHosting.createPR).toHaveBeenCalledWith(expect.objectContaining({ title: "Fix authentication bug" }));
  });
  ...
  it("does not decorate PR title when external_ref is null", async () => {
    ...
    expect(fakeGitHosting.createPR).toHaveBeenCalledWith(expect.objectContaining({ title: "Test task" }));
  });
});
