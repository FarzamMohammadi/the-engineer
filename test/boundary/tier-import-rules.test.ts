 ...
 */
function getImportTier(
  importPath: string,
  fromFile: string,
): "core" | "adapters" | "schemas" | "external" | null {
...
describe("three-tier import rules", () => {
    ...
    for (const file of sourceFiles) {
      ...
      for (const imp of imports) {
        ...
        if (!allowedTiers.has(importTier)) {
          const rel = relative(SRC_ROOT, file);
          violations.push(
            `${rel}:${String(imp.line)} imports from ${String(importTier)}: ${imp.path}`,
          );
        }
      }
    }
  ...
  it("core files never import from test helpers or plugins", () => {
    ...
    for (const file of sourceFiles) {
      ...
      const imports = extractImports(file);
      for (const imp of imports) {
        if (imp.path.includes("test/") || imp.path.includes("fake-plugin")) {
          const rel = relative(SRC_ROOT, file);
