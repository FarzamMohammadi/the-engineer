...
function createManifest(type: AdapterType, id: string, overrides?: Partial<PluginManifest>): PluginManifest {
...
describe("Registry", () => {
  ...
  describe("register", () => {
    ...
    it("rejects duplicate plugin ID", () => {
      ...
      const result = registry.register(createManifest("trigger", "dup-id"), new FakeTriggerPlugin());
    ...
    it("logs registration via observer", () => {
      ...
      expect(infoSpy).toHaveBeenCalledWith("Plugin registered", expect.objectContaining({ pluginId: "test-trigger" }));
    });
  });
  ...
  describe("initializePlugin", () => {
    ...
    it("logs successful initialization with timing via observer", async () => {
      ...
      expect(infoSpy).toHaveBeenCalledWith("Plugin initialized", expect.objectContaining({ pluginId: "t1" }));
    });
  });
  ...
  describe("health state machine", () => {
    ...
    it("transitions unhealthy → healthy on successful check", async () => {
      ...
      instance.setUnhealthy(true);
      await registry.healthCheckAll();
      expect(assertDefined(registry.getHealthRecord("t1")).state).toBe(PluginHealthStates.unhealthy);
    ...
    it("emits health.plugin_failed on threshold breach", async () => {
      ...
      handle.assertEventEmitted("health.plugin_failed", (p) => p["plugin_id"] === "t1" && p["threshold"] === 2);
    });
