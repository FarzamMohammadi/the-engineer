...
import { AgentRunRejectedError, AgentUnavailableError, NoAgentPluginError, WorkspaceNotReadyError } from "./errors.js";
...
function updateManifest(input: UpdateManifestInput): void {
  ...
  try {
    ...
    // Best-effort — manifest write failure never blocks the agent run
  }
}
...
/** Input for {@link AgentRunner.runPhase}. */
...
/** Agent invocation, cost tracking, and response validation. */
export interface AgentRunner {
  /** Run the agent through ActionPipeline. Throws on rejection or no plugin. */
  run(prompt: string, taskId: string, systemPrompt?: string | null): Promise<AgentRunResult>;
  /** Run a phase via CLI-native invocation. Single CLI call, file-based routing. */
  runPhase(input: RunPhaseInput): Promise<PhaseOutput>;
  /** Emit cost.incurred event from an agent run result. */
  emitCostIncurred(taskId: string, result: AgentRunResult): void;
}

// ── Factory ─────────────────────────────────────────────────────────────────

/** Create an AgentRunner bound to the given OrchestratorContext. */
export function createAgentRunner(ctx: OrchestratorContext): AgentRunner {
  /** Single agent run attempt without retry. */
  async function runOnce(
    ...
    if (pipelineResult.outcome !== "executed") {
      ...
      throw new AgentRunRejectedError(pipelineResult.outcome, reason);
    }

    return pipelineResult.result;
  }

  /** Alternatives recorded at each agent retry decision point. */
  ...
  /** Run the agent with retry for transient failures. */
  ...
  async function run(
    ...
    for (let attempt = 0; attempt < MAX_AGENT_RETRIES; attempt++) {
      try {
        return await runOnce(prompt, taskId, systemPrompt, traceOutputPath);
      } catch (error) {
        lastError = error;
        if (!isRetryableError(error)) {
          ctx.observer.recordDecision(
            "agent_retry",
            `Agent attempt ${String(attempt + 1)} failed for task ${taskId}`,
            AGENT_RETRY_OPTIONS,
            "abort",
          ...
          ctx.observer.warn("Agent run failed (non-retryable) — not retrying", {
            taskId,
            attempt: attempt + 1,
        ...
        if (attempt < MAX_AGENT_RETRIES - 1) {
          const delay = AGENT_RETRY_BASE_MS * 2 ** attempt;
          ctx.observer.recordDecision(
            "agent_retry",
            `Agent attempt ${String(attempt + 1)} failed for task ${taskId}`,
            AGENT_RETRY_OPTIONS,
            "retry",
            `Transient error — retrying after ${String(delay)}ms (${String(attempt + 1)}/${String(MAX_AGENT_RETRIES)})`,
            1,
            { task_id: taskId },
          );
          ctx.observer.warn("Agent run failed (transient) — retrying", {
            taskId,
            attempt: attempt + 1,
    ...
    ctx.observer.recordDecision(
      "agent_retry",
      `Agent attempt ${String(MAX_AGENT_RETRIES)} failed for task ${taskId}`,
      AGENT_RETRY_OPTIONS,
      "abort",
      `Retry budget exhausted (${String(MAX_AGENT_RETRIES)} attempts)`,
      1,
      { task_id: taskId },
    );
    ctx.observer.error("Agent retries exhausted", {
      taskId,
  ...
  function emitCostIncurred(taskId: string, result: AgentRunResult): void {
  ...
  async function runPhase(input: RunPhaseInput): Promise<PhaseOutput> {
    const {
      phase,
      taskId,
      systemPrompt,
    ...
    try {
      result = await run(prompt, taskId, systemPrompt, tracePath);
    } catch (error) {
