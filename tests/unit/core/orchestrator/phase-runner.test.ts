...
import type { Phase, PhaseOutput } from "../../../../src/schemas/orchestrator.js";
...
function createHandlersThatReturn(outputs: Map<Phase, PhaseOutput>) {
  return createPhaseHandlerRegistry(
    Object.fromEntries(
      PHASE_SEQUENCE.map((phase) => [phase, vi.fn(() => Promise.resolve(outputs.get(phase) ?? makeOutput(phase)))]),
...
describe("PhaseRunner", () => {
  describe("runPhasePipeline", () => {
    ...
    it("emits loopback alert when threshold exceeded", async () => {
      ...
      const result = await runPhasePipeline(createDispatch(), createState({ loopbackCount: 3 }), deps);

      // Should proceed past self_review (alert emitted, no loop)
      expect(result.outcome).toBe("completed");
      expect(ctx.notifications.notify).toHaveBeenCalledWith(expect.objectContaining({ kind: NotificationKinds.alert }));
    });
  ...
  describe("config-driven behavior", () => {
    it("respects custom rrpir.max_review_loopbacks from config", async () => {
      ...
      const result = await runPhasePipeline(createDispatch(), createState({ loopbackCount: 4 }), deps);
  ...
  describe("universal fallback routing", () => {
    ...
    it("awaits outreach delivery before blocking", async () => {
      ...
      try {
        ...
        (ctx.registry.getPluginsByType as ReturnType<typeof vi.fn>).mockReturnValue([mockCommPlugin]);
        ...
        const result = await runPhasePipeline(createDispatch(), createState({ thoughtsDir: "thoughts/test" }), deps);
    ...
    it("persists return_to_phase when blocking from a fallback", async () => {
      ...
      const reqHandler = vi.fn(() => {
        reqCalls++;
        if (reqCalls === 2) {
          return Promise.resolve(makeOutput(Phases.requirements_gathering, { status: "need_more_info" }));
        }
        return Promise.resolve(outputs.get(Phases.requirements_gathering) ?? makeOutput(Phases.requirements_gathering));
      });
      const handlerFns = Object.fromEntries(
        PHASE_SEQUENCE.map((phase) => [
          phase,
      ...
      expect(ctx.taskEngine.updateTaskField).toHaveBeenCalledWith("task-001", "return_to_phase", Phases.planning);
    });
    ...
    it("reads return_to_phase from task on re-dispatch", async () => {
      ...
      expect(ctx.taskEngine.updateTaskField).toHaveBeenCalledWith("task-001", "return_to_phase", null);
    });
    ...
    it("does NOT persist return_to_phase when requirements_gathering blocks on its own", async () => {
      ...
      const returnToPhaseWrites = updateCalls.filter((c: unknown[]) => c[1] === "return_to_phase" && c[2] !== null);
