...
**PR status.** Fetches the PR and the combined commit status for the head SHA. Maps GitHub's state to a simplified `open | closed | merged` enum. Mergeability maps from GitHub's `mergeable` boolean and `mergeable_state` string onto `merge_state`: `mergeable === false` is a definitive textual conflict (`conflicting`, reworked); `mergeable === null` is the host still computing (`unknown`, a wait). Otherwise the PR is mergeable in _shape_, and `mergeable_state` decides whether the host will actually **complete** the merge — an **allowlist**: `clean`, `unstable`, `has_hooks`, and `behind` map to `mergeable` (the host will merge them); `unknown` or absent maps to `unknown` (wait and re-check — never merge on an unverified state); **everything else** — `blocked`, `draft`, and any state GitHub adds later — maps to `blocked`, which Core hands to the owner and **never** reworks (the code is fine; only the merge is gated). `mergeable_state` must be consulted before collapsing `mergeable === true`, because a blocked PR still reports `mergeable === true` and a naive "true → mergeable" would mask the block. `behind` is deliberately **not** blocked — it is an out-of-date branch, not a conflict, and GitHub merges it when protection does not require up-to-date branches. The allowlist is the fail-safe direction: a merge attempt first pushes a thoughts-cleanup commit (which can dismiss a formal approval), so an unrecognized state hands off — recoverable — rather than attempting a merge the host may refuse. CI check status is determined by querying both the Status API (`repos.getCombinedStatusForRef`) and the Checks API (`checks.listForRef`) in parallel, then combining them worst-state-wins (so a single failing check anywhere fails the gate). If the lookup itself errors transiently (a network blip, `ECONNRESET`, dropped connection, or rate-limit), the plugin reports `checks_state: unknown` — never `failing` — because it could not read the checks; it does not salvage a partial answer when one of the two calls fails, so it never claims `passing` on an unverified lookup. Core treats `unknown` as a wait, so a blip leaves the task waiting and re-polling instead of triggering a phantom rework.
