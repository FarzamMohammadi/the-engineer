...
export function gatherInstructions(directory: string): string {
  return [
    ...
    "",
    "3. State what done looks like. Under a `## Acceptance Criteria` heading, write the concrete, checkable conditions a reviewer would verify to call this task complete. Draw them from what the task says and what the code shows — not from what you wish it said. These criteria are the end-state you interrogate in the next step. Record the same list — one string per condition — in `details.acceptance_criteria`: this is the structured handoff the review gates on and the dashboard shows, so it must match the prose you write here.",
    "",
