...
describe("storeKnowledge", () => {
  ...
  it("stores user-scope knowledge with null repo_scope", () => {
    ...
    const entry = sessionMemory.storeKnowledge(makeKnowledgeInput({ scope: KnowledgeScopes.user, repoScope: null }));
...
describe("getKnowledge", () => {
  it("returns only non-superseded entries", () => {
    ...
    const old = sessionMemory.storeKnowledge(makeKnowledgeInput({ key: "old-key", body: "old body" }));
    const newer = sessionMemory.storeKnowledge(makeKnowledgeInput({ key: "new-key", body: "new body" }));
