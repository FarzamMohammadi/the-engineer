...
function buildDemoPrepInstructions(): string {
  return section(
    ...
    [
      ...
      "",
      "3. Write a `pr-description.md` proportional to the change:",
      "   - What changed and why (always)",
      "   - How to test (always — concrete steps a reviewer can follow)",
      "   - Technical approach (only if non-obvious or multiple approaches were evaluated)",
      "   - Breaking changes or deployment notes (only if applicable)",
      "   - Key decisions (only if genuine tradeoffs were made)",
      "",
      "   Match depth to complexity. A small, focused change gets a short description. A system redesign gets a full narrative. If every PR reads like an architecture doc, none of them communicate effectively.",
      "",
