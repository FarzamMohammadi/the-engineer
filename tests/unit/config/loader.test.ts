...
const FIXTURES_DIR = path.resolve(import.meta.dirname, "../../fixtures/configs");
...
describe("parseDurations", () => {
  ...
  it("handles nested duration fields", () => {
    ...
    const result = parseDurations({ notification: { suppress_window_ms: "5m", channel: "telegram" } }, schema);
...
describe("loadConfigDir", () => {
  it("loads all configs from directory with defaults for missing files", () => {
    ...
    expect(result.bundle.orchestrator.review.lenses).toEqual(["self-review"]);
