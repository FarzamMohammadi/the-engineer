...
import type { StoreKnowledgeInput } from "../../../../src/core/interfaces/session-memory.interface.js";
import { KnowledgeStore } from "../../../../src/core/session-memory/knowledge.js";
import { KnowledgeConfidences, KnowledgeDomains, KnowledgeScopes } from "../../../../src/schemas/session-memory.js";
...
describe("KnowledgeStore", () => {
  ...
  it("stores user-scope knowledge with null repo_scope", () => {
    setup();
    const entry = knowledge.storeKnowledge(makeInput({ scope: KnowledgeScopes.user, repoScope: null }));
  ...
  it("throws for non-existent ID on confirm", () => {
    setup();
    expect(() => knowledge.confirmKnowledge("nonexistent")).toThrow('Knowledge entry "nonexistent" not found');
  });
});
