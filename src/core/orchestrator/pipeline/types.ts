...
export interface PhaseDefinition {
   ...
   * `PREMISE_CONFLICT_CATEGORY` (defined in `schemas/config`): the agent's own investigation contradicts
   * the task's premise (or shows the need already met), which no later phase recovers — so the owner is asked proceed/redirect/
