...
A task's detail page opens on its title, state, and current position in the pipeline, plus a **Cancel** button while the task is still cancellable. If a blocked task needs you, an amber banner sits at the top with the block details and a response box — see [Responding to a blocked task](#responding-to-a-blocked-task). Below that, six tabs break the task down:
