...
## External Trace Export (OpenTelemetry)

**Current state:** Every dispatch is already one end-to-end trace. A root `task_execution` span anchors it, and every phase transition, sub-phase result, agent run, decision, verdict, and external action nests under it — one correlation id per dispatch, a parent reference threading the tree. The whole trace is stored locally (structured spans in SQLite, large payloads — prompts, responses, gate output — in a content-addressable blob store) and rendered by the bundled dashboard. There is no export to an external tracing system.

**When it becomes relevant:** When the owner wants to view traces in a dedicated observability tool — Jaeger, Tempo, Honeycomb, Grafana, any OTLP-compatible backend — instead of or alongside the bundled dashboard. The signals: traces that span more than one process or machine; a desire to correlate The Engineer's traces with the CI, the git host, or the agent provider's own telemetry; or simply richer trace UIs (flame graphs, service maps, latency analysis) than the bundled dashboard aims to build.

**What it enables:** The same task seen as a flame graph in Tempo or a waterfall in Honeycomb, on top of the local dashboard — and cross-system correlation when The Engineer is one service among several.

**Key context — most of the work is already done.** The hard part (modelling each dispatch as a single nested trace with stable correlation) exists. The observation model maps almost one-to-one onto OpenTelemetry: an observation is a span (correlation id → trace, parent reference → parent span, start/duration → timing, input/output/metadata → attributes, status → span status); a recorded decision is a span carrying its alternatives and reasoning as attributes (or a span event); a blob reference becomes a link to large content. Export is a *projection* of the existing tree, not new instrumentation. The one wrinkle is the blob store: OTLP attributes have size limits, so large payloads stay as references (links), never inlined.

**Migration path:** A new, opt-in exporter subscribes to the same real-time observation stream the dashboard's live view already consumes, maps each observation to an OTLP span, and ships to a configured collector endpoint. It is additive — off by default, enabled by config (endpoint + headers), changing nothing about how the pipeline emits; the dashboard keeps working unchanged. A containerized collector (an OTel Collector plus a trace backend, as an optional local bundle) can ship for owners who want the full UI without standing up their own, but it is not required — any OTLP endpoint works. Keep the projection in one boundary module so the observation model stays the source of truth and the OTel mapping is swappable. Treat it as best-effort telemetry: a collector being down must never wedge a task (fire-and-forget with its own error isolation, like every other degradable dependency).
